...
| Limitations | `paper_draft.md`, `claim_ledger.md` | no full 511-language claim; target official task membership is partial but measured downstream claim awaits tail-materialization repair; external reference caveat | ready | update only if coverage or blocked metrics change |
