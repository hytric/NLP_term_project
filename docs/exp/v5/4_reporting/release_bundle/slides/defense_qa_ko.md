...
## Q17. target10 downstream coverage가 0/10이면 downstream task를 충실히 이행한 것인가?
...
다만 현재 local downstream data가 target10을 거의 포함하지 않으므로, target10 downstream
improvement는 말하지 않는다.
NER output에는 `fur_Latn` 단일 actual evaluated intersection이 있지만, 이것은
materialized target10 coverage나 target10-wide downstream evidence가 아니라
단일 target-language row로만 보고한다. 이 row는 XLM-R, Glot500-base, v5-random 모두
aggregation에 들어갔지만, target10-wide downstream evidence로 승격하지 않는다.
