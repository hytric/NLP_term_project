...
\title{XLM-R 기반 저자원 언어 Vocabulary Extension:\\ Embedding Initialization 방법론 비교와 수렴 분석}
\author{Jongha Kim}
...
\begin{abstract}
\noindent
XLM-R은 100개 언어를 학습한 강력한 multilingual masked language model(MLM)이지만, 학습에 포함되지 않은 tail language에서는 한 단어가 여러 subword로 과도하게 쪼개지고(over-fragmentation) 어휘·형태 정보가 표현 공간에 정렬되지 않는 문제가 남는다. Glot500은 mixed corpus로 SentencePiece unigram tokenizer를 재학습해 vocabulary를 확장하고 확장 모델을 MLM으로 continued pretraining하여 이 문제를 완화했다. 그러나 vocabulary를 확장하면 embedding matrix 뒤에 기존에 없던 새 token row가 추가되는데, 이 row를 어떤 벡터로 시작하느냐(embedding initialization)는 tokenizer 설계와 독립적인 modeling 선택임에도 대개 임의의 random으로 처리되고 체계적으로 비교된 적이 드물다.

본 보고서는 이 초기화 선택만을 통제 변인으로 두고 다섯 방법을 비교한다: \rand, \meanm, \fvt, 그리고 우리가 추가한 두 변형 \wfvt, \famm. \texttt{xlm-roberta-base} 위에서 92개 XLM-R-seen replay 언어와 7개 XLM-R-unseen target(Target7)을 사용하고, tokenizer는 Glot500 방식으로 고정한 채 초기화만 바꿔 동일한 corpus·objective·schedule·evaluation protocol로 50K-step까지 continued MLM pretraining을 수행한다. 평가는 Glot500 downstream suite(pseudoperplexity, Tatoeba/Bible retrieval, NER, roundtrip alignment, text classification)로 하며, 시작점인 XLM-R-B와 XLM-R-L을 baseline으로 둔다.

세 가지를 관찰한다. (1) 확장 tokenizer는 Target7의 tokens/word를 2.204에서 1.592로 27.75\% 낮춘다. (2) 같은 tokenizer를 공유함에도 초기화에 따라 50K 수렴 loss가 갈리며, FVT 계열이 가장 낮고 \meanm\ 은 \rand\ 보다도 높다. (3) 50K downstream에서 우리가 추가한 두 변형이 target PPPL·Tatoeba·NER·roundtrip에서 최고이고, 특히 \famm\ 은 NER에서 ablation 최고이며, 학습 초반 앞서던 plain \fvt\ 를 수렴 구간에서 추월한다. 요컨대 저자원 vocabulary adaptation에서 embedding initialization은 수렴과 downstream 성능을 실제로 바꾸는 중요한 선택이며, source subtoken/계통 정보를 재사용하는 FVT 계열이 지속적으로 유리하다.
\end{abstract}
...
\item tokenizer를 고정하고 초기화만 바꾸는 통제된 5-way ablation을 설계하고, 표면 길이 가중 \wfvt\ 와 계통 prior \famm\ 두 방법을 제안한다.
\item 초기화가 50K-step 수렴 loss와 downstream 성능을 유의하게 바꿈을 보인다. 특히 두 변형이 수렴 구간에서 plain FVT를 추월하며, \famm\ 이 target 지표에서 가장 좋다.
\item 문장 표현 공간의 계통 구조가 학습 과정에서 어떻게 형성되는지 step별로 분석한다.
\end{enumerate}
...
\rand·\meanm\ 은 모든 축에서 하위이고 개선은 전부 FVT 계열에서 나온다. \famm\ 이 target downstream 최강(PPPL·Tatoeba·NER 최고)이나 전체 corpus loss는 3위로 target에 특화된다. \wfvt\ 가 가장 균형(loss·Roundtrip 최고, 나머지 상위권)이다. plain \fvt\ 는 ``빠른 출발, 조기 포화'' 프로파일이다. 결론적으로 target 성능이 목표면 \famm, 전반적 균형이 목표면 \wfvt\ 가 최선이다.

% =====================================================================
\section{결론 및 한계}

\paragraph{결론.} \texttt{xlm-roberta-base}에 Glot500식 vocabulary extension을 적용하고 새 embedding row 초기화를 다섯 방법으로 50K-step까지 통제 비교했다. (1) 확장 tokenizer는 Target7 fragmentation을 27.75\% 줄였다. (2) 같은 tokenizer·corpus에서도 초기화에 따라 50K 수렴 loss가 갈렸고 FVT 계열이 최저, \meanm\ 이 \rand\ 보다도 높았다. (3) 50K downstream에서 우리가 추가한 \wfvt·\famm\ 이 target PPPL·Tatoeba·NER·Roundtrip에서 최고였고, 학습 초반 앞서던 plain FVT를 수렴 구간에서 추월했다. (4) 문장 표현 공간에서는 script보다 family 구조가 강하게 나타났고 그 구조는 학습 초반에 먼저 형성되었다. 요컨대 embedding initialization은 저자원 vocabulary adaptation에서 수렴과 downstream을 실제로 바꾸며, source subtoken/계통 정보를 재사용하는 FVT 계열(특히 target이면 \famm, 균형이면 \wfvt)이 지속적으로 유리하다.

\paragraph{한계.} 계산 비용이 큰 일부 sequence-labeling(POS) 평가는 5-way 완주하지 못해 제외했다(compute 제약, future work). task별 target coverage가 다르고(NER 100문장 subset, Bible floor, Text head/EN-only), 50K는 완전 수렴이 아니라 절대 성능은 full Glot500에 미달한다. \wfvt·\famm\ 은 local variant이며 단일 run이라 seed 분산은 충분히 평가되지 않았다. family similarity 분석은 전체 99개가 아니라 38개 language subset에 대한 것이다.
...
그림~\ref{fig:strips}는 다섯 초기화 각각에 대해 step 10K/20K/30K/40K/최종(\rand\ 43K, \meanm\ 45K, 나머지 50K)의 문장 임베딩 2D map을 모두 보인 것이다. 색=language family, 삼각형=tail(Target7), 원=head/reference. 모든 방법에서 계통 군집 구조가 조기에 형성되어 step에 걸쳐 안정적으로 유지된다.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth]{strip_random.png}\\[3pt]
\includegraphics[width=\linewidth]{strip_mean.png}\\[3pt]
\includegraphics[width=\linewidth]{strip_fvt.png}\\[3pt]
\includegraphics[width=\linewidth]{strip_weighted_fvt.png}\\[3pt]
\includegraphics[width=\linewidth]{strip_family_mean.png}
\caption{초기화 방법별·step별 2D embedding map 전체. 위에서부터 \rand, \meanm, \fvt, \wfvt, \famm; 각 행은 왼쪽부터 step 10K, 20K, 30K, 40K, 최종.}
\label{fig:strips}
\end{figure}
